...
from forest_fire.agent import WindPackage
...
class ForestFire(mesa.Model):
        ...
        self.datacollector = mesa.DataCollector(
            {
                ...
                "Fine Resistant": self.count_fine_Resistant, 
                "Wind Activations": self.count_wind_activation # Counting fine Resistant trees
            }
        )
    ...
    @staticmethod
    def count_wind_activation(model):
        """Count the number of wind activations that successfully spread fire."""
        return sum(1 for agent in model.schedule.agents if isinstance(agent, WindPackage) and agent.activated)
